...
print(HotellingsT2Test(cbind(tbl$V1, tbl$V2, tbl$V3, tbl$V4, tbl$V5) ~ tbl$V6, test = "chi"))
```
...
```{r}
# Using MASS package to get qda results
dat3 <- dat2
dat3$d2 <- as.numeric(dat3$d2)
train <- createDataPartition(dat3$d2, p = 0.6, list = FALSE)
training <- dat3[train,]
testing <- dat3[-train,]
qda_model <- qda(d2~PC1+PC2 ,data = training, CV = T)
a <- table(training$d2, qda_model$class)
print("Confusion Matrix")
print(a)
print(paste("Accuracy: ", sum(diag(a))/nrow(training)))
```
```{r}
qda_model2 <- qda(d2 ~ PC1 + PC2, data = training)
qda.predict <- predict(qda_model2, testing)
testing$qda <- qda.predict$class
confQda <- table(testing$qda, testing$d2)

print("Confusion Matrix")
print(confQda)
print(paste("Accuracy: ", sum(diag(confQda))/nrow(testing)))
```
